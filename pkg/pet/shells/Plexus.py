...
# declaration
class Plexus(pyre.plexus, family="pet.shells.plexus"):
    """
    ...
    def pyre_banner(self):
        ...
        """
        # the project header
        yield from textwrap.dedent(pet.meta.banner).splitlines()
        # the doc string
        yield from self.pyre_showSummary(indent="")
        # the authors
        yield from textwrap.dedent(pet.meta.authors).splitlines()
        # all done
        return
    ...
    def pyre_mountApplicationFolders(self, pfs, prefix, **kwds):
        ...
        # if all goes well and we reach the intended folder without errors
            ...
            self._ux = pet.ux.dispatcher(plexus=self, docroot=docroot, pfs=pfs)
